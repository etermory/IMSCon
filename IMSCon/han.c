...
unsigned int ks_table[MAX_KSCODE] =
{
...
unsigned int sg_table[SINGLENUM] =
{
...
unsigned int search(unsigned int val, unsigned int range)
{
...
void tg_to_ks(unsigned char *c1, unsigned char *c2)
{
     unsigned int index, code ;

     code = (*c1 << 8) | *c2 ;
     if ((index = search(code, MAX_KSCODE)) != MAX_KSCODE) {
         *c1 =  (index / 94) + 0xb0;
         *c2 =  (index % 94) + 0xa1;
     } else {
          *c1 = *c2 = SPACE;
          for (index = 0; index < SINGLENUM; index++) {
               if (sg_table[index] == code) {
                   *c1 = 0xa4;
                   *c2 = 0xa1 + index;
                   break;
               }
...
void ks_to_tg(unsigned char *c1, unsigned char *c2)
{
     unsigned int c;

     if (*c1 < 0xb0 || 0xc8 < *c1 || *c2 < 0xa1 || 0xfe < *c2) {
         c = 0x2020;
     } else {
         if (*c1 == 0xa4) {
             if (0xa1 <= *c2 && *c2 <= 0xd3) {
...
void han_conv(int flag, char* src, char* dst)
{
    char buf[256] = { 0, };
    strcpy(buf, src);

    unsigned char is_han, *c1, *c2;
    int pos = 0;
    while (buf[pos] != '\0')
    {
        c1 = (unsigned char*)(buf + pos);
        pos++;

        if (flag == KS) {
            is_han = ((0xb0 <= *c1 && *c1 <= 0xc8) || (*c1 == 0xa4));
        }
        else {
            is_han = (0x80 <= *c1);
        }
